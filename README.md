...
— Gemini и Claude иногда игнорируют любую кастомную думалку — просто перегенерируйте. Или добавьте свой тег <think> в префикс; для моего пресета это think. Префикс — это поле «Start response with…». Вывод think, появляющийся в ответах, должен вырезаться через regex на think (включён в пресет)
