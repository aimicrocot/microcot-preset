...
— Claude 4.1–4.5 Sonnet/Opus ~ пригодны, но по своей природе более рассеянные и склонные к потере памяти (Memorybook и Summaries могут помочь)

— GPT 5.1+ ~ пригодны, так как новые GPT имеют более слабые фильтры, но если цензура становится агрессивной, включите «Anti-censor Think» или используйте синонимы для откровенного контента

## Что здесь интересного?

╰► Anti-censor think + anti-censor combo модули

— Пресет можно использовать в двух режимах: без <think> и с <think> (включите 2 модуля Anti-censor think)

— Когда? Если вы видите цензуру. По моим тестам, <think> «тупит» модель меньше и эффективнее, чем space regex’ы, brainf*cks и длинные NSFW-промпты

— Gemini и Claude иногда игнорируют любой кастомный think — просто перегенерируйте. Или добавьте свой тег think в префикс; для моего пресета это think. Префикс — это поле «Start response with…». Вывод think, появляющийся в ответах, должен вырезаться через regex по think (включён в пресет)
...
<summary> 
  Нажмите сюда:
</summary>— Microcot ~ лучше включать выборочно, когда хотите обращаться к нему через OOC. Ролеплеер и комментатор, милый зумер с кошачьими привычками. Может слегка помочь с фильтрами цензуры
...
— Writing format ~ если во время русского RP вы хотите использовать прямые кавычки ("") вместо длинных тире (—) и ёлочек («») (спасибо @xvcrrd)

— Post up to 250 words ~ включите, если с thinker’ом или без него он пишет «стены текста» (может писать больше 250, но не слишком много)

— HTML/CSS & Kitten CSS ~ используйте с regex! Супер-лаконичный, но эффективный промпт, работает как большие и также генерирует изображения и иногда интерактив. Включайте только при необходимости, чтобы не замедлять модель. Если результат странный — удалите пост и сгенерируйте снова. Хотите реально крутой CSS? Поставьте Reasoning на максимум

— Info table ~ включайте, когда персонажи / локации / одежда часто меняются

— Anti-echo ~ попытка сократить эхо более лаконично и с меньшей деградацией модели (подсмотрено у @maybee0rnot)

— Anti-omniscience ~ снижает шанс того, что персонажи знают друг о друге больше, чем было явно указано в игре
...
— No dirty talks ~ специально для меня, надоело

— No lists ~ для GPT и иногда Gemini

— 3rd or 1st person ~ включите, если вы начинаете RP без стартового сообщения и даёте боту самому сгенерировать открытие (чтобы он сразу писал в нужном лице)

— Chat style RU & EN ~ ОРГАНИЧНЫЕ, короткие сообщения, как в мессенджере

— REALLY wild RU & EN comments on posts ~ максимальная аутентичность

— Plot boost ~ включайте, когда нужно продвинуть сюжет. Не скучно, но без роялей в кустах

— Anti-censor below ~ убирает фильтры цензуры; Gemini 3 без него не работал

— Anti-censor <del> ~ для комбо с Anti-censor below + используйте Regex <del>
...
</details>Лайфхаки и благодарности

— Для максимального интеллекта модели используйте карточки персонажей до 800 токенов БЕЗ списков, скобок, markdown, W++, или Boostyle. Не используйте infotabs/HTML/CSS постоянно и перезапускайте RP с саммари примерно на 25k контекста. Лично я ролю на английском и держу Reasoning на минимуме. Почему мой пресет именно такой: https://rentry.org/promting
